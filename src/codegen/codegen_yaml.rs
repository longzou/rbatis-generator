use std::{fs::File, collections::btree_map::VacantEntry, vec};

use change_case::{snake_case, pascal_case};
use substring::Substring;
use yaml_rust::{Yaml, yaml};

use super::{RustStruct, RustStructField, RustFileImpl, RustFunc};
...
pub fn parse_yaml_as_file(conf_path: &str, filename: &String) -> RustFileImpl {
  ...
  RustFileImpl { file_name: filename.clone(), mod_name: "conf".to_owned(), caretlist: vec![], usinglist: usinglist, structlist: sts, funclist: vec![] }
}
...
fn generate_load_from_node_for_struct(st: &mut RustStruct) {
  ...
  for rstfd in st.fields.clone() {
    match rstfd.field_type.clone().as_str() {
      "String" => {
        body.push(format!("{}: match node[\"{}\"].as_str() {{", rstfd.field_name, rstfd.column_name));
        body.push(format!("Some(st) => Some(st.to_string()),"));
        body.push(format!("None => None,"));
        body.push(format!("}},"));
      },
      "i64" => {
        body.push(format!("{}: node[\"{}\"].as_i64(),", rstfd.field_name, rstfd.column_name));
      },
      "f64" => {
        body.push(format!("{}: node[\"{}\"].as_f64(),", rstfd.field_name, rstfd.column_name));
      },
      "bool" => {
        body.push(format!("{}: node[\"{}\"].as_bool(),", rstfd.field_name, rstfd.column_name));
      },
      "Vec<String>" => {
        body.push(format!("{}: match node[\"{}\"].as_vec() {{", rstfd.field_name, rstfd.column_name));
      ...
      _ => {
        ...
        if  fdtype.starts_with("Vec<") && fdtype.ends_with(">") {
          ...
          body.push(format!("{}: match node[\"{}\"].as_vec() {{", rstfd.field_name.clone(), rstfd.column_name.clone()));
          body.push(format!("Some(vst) => {{"));
          body.push(format!("let mut vlist = vec![];"));
          body.push(format!("for xst in vst.clone() {{"));
          body.push(format!("let sop = {}::load_from_node(&xst);", field_type.clone()));
          ...
          if rstfd.is_option {
            body.push(format!("{}: Some({}::load_from_node(&node[\"{}\"])), ", rstfd.field_name.clone(), rstfd.field_type.clone(), rstfd.column_name.clone()));
          } else {
            body.push(format!("{}: {}::load_from_node(&node[\"{}\"]), ", rstfd.field_name.clone(), rstfd.field_type.clone(), rstfd.column_name.clone()));
          }
        }
      }
    }
...
fn generate_load_from_yaml_for_struct(st: &mut RustStruct) {
  ...
  body.push(format!("let docs = yaml::YamlLoader::load_from_str(&s).unwrap();"));
  body.push(format!("let doc = &docs[0];"));

  body.push(format!("let conf = {}::load_from_node(doc);", st.struct_name.clone()));

  for fd in st.fields.clone() {
    body.push(format!("self.{} = conf.{}.clone();", fd.field_name.clone(), fd.field_name.clone()));
  }
...
fn generate_single_get_for_struct(st: &mut RustStruct) {
  ...
  body.push(format!("static mut CONF: MaybeUninit<Mutex<{}>> = MaybeUninit::uninit();", st.struct_name.clone()));
  body.push(format!("// Once带锁保证只进行一次初始化"));
  body.push(format!("static ONCE: Once = Once::new();"));

  body.push(format!("ONCE.call_once(|| unsafe {{"));
  body.push(format!("CONF.as_mut_ptr().write(Mutex::new({}::default()));", st.struct_name.clone()));
  body.push(format!("}});"));
